...
export default function TeamsPage() {
  ...
  // Initial Teams Data with Captain Name, Roster, Game Tag, and Stats
  ...
  const handleCreateTeam = (e) => {
    ...
    setNewTeam({ name: "", game: "BGMI", captain: "", membersInput: "" });
  };
    ...
    <div className="min-h-screen p-8 bg-slate-950 text-white max-w-7xl mx-auto">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-slate-400 mt-1">
            Registered Teams & Esports Rosters in GameHub Arena
          </p>
        </div>

        {/* Admin Protected Create Team Button */}
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {teams.map((team) => (
          ...
          >
            <div>
              {/* Primary Game Tag & Rank Badge */}
              <div className="flex justify-between items-center">
                <span className="text-xs font-semibold px-2.5 py-1 rounded bg-indigo-500/20 text-indigo-300">
                  {team.game}
                </span>
                <span className="text-xs font-bold px-2.5 py-1 rounded bg-yellow-500/20 text-yellow-400 border border-yellow-500/30">
                  Rank {team.rank}
                </span>
              </div>
          ...
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl max-w-md w-full text-white shadow-2xl relative">
            ...
            <div className="mt-4 space-y-2 text-sm text-slate-300">
              <p>
                ...
                <span className="text-indigo-300 font-semibold">{selectedTeam.captain}</span>
              </p>
              <p>
                <strong>Rank & Performance:</strong>{" "}
                <span className="text-yellow-400 font-bold">Rank {selectedTeam.rank}</span> (Win Rate:{" "}
                <span className="text-green-400 font-bold">{selectedTeam.winRate}</span>)
              </p>
              <p>
                <strong>Matches Played:</strong> {selectedTeam.matchesPlayed} Matches
              </p>
            </div>
            ...
            <div className="mt-5 border-t border-slate-800 pt-4">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">
                Squad Roster ({selectedTeam.members.length} Players)
              </h3>
              <ul className="space-y-2">
                {selectedTeam.members.map((m, idx) => (
                  <li
                    key={idx}
                    className="bg-slate-950 p-2.5 rounded-xl border border-slate-800/80 text-sm flex items-center gap-2.5"
                  >
        ...
        <div className="fixed inset-0 bg-black/85 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl max-w-sm w-full text-white shadow-2xl relative text-center">
            ...
            <p className="text-xs text-slate-400 mt-1">
              Enter admin password to create or join a new team roster.
            </p>
      ...
      {isCreateModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl max-w-md w-full text-white shadow-2xl relative">
            ...
            <h2 className="text-2xl font-bold text-indigo-400 mb-4">Create / Join Team</h2>
            ...
            <form onSubmit={handleCreateTeam} className="space-y-4">
              ...
              <div>
                <label className="text-xs text-slate-400 block mb-1">
                  Squad Roster Members (Comma Separated)
                </label>
                <input
                  type="text"
                  placeholder="Deepika (Captain), Samar, Parth, Vaibhav"
                  value={newTeam.membersInput}
                  onChange={(e) => setNewTeam({ ...newTeam, membersInput: e.target.value })}
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-sm focus:outline-none focus:border-indigo-500"
                />
              </div>
